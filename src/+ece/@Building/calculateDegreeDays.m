function calculateDegreeDays(obj)
% CALCULATEDEGREEDAYS: Method to calculate the DegreeDay values for a
% Building.
...
%% Initialize Weather DataTable
% Starts as general WeatherData from Location, then will expand.
obj.BuildingWeatherTable = obj.Location.WeatherDataTable;

%add columns to the weather data table for the month, hour, and day number
obj.BuildingWeatherTable.Day = day(obj.BuildingWeatherTable.Date);
obj.BuildingWeatherTable.Month = month(obj.BuildingWeatherTable.Date);
obj.BuildingWeatherTable.Hour = hour(obj.BuildingWeatherTable.Time);

%assign a day of the year number to each hour to use for heating/cooling
%seasons
obj.BuildingWeatherTable.dayNumber = floor((1:1/24:365.96)');

%convert degrees C to F and wind speed m/s to mph
obj.BuildingWeatherTable.DryBulb_F = obj.BuildingWeatherTable.DryBulb .* 1.8 + 32;
obj.BuildingWeatherTable.Rhum = obj.BuildingWeatherTable.Rhum;
obj.BuildingWeatherTable.wSpeed_mph = obj.BuildingWeatherTable.wSpeed * 2.237;
%enthalpy setpoints based on cooling temp setpoint and target RH
en1Setpt = ece.Building.calculateEnthalpyAirH20(clg1Setpt, targetRHsummer);
en2Setpt = ece.Building.calculateEnthalpyAirH20(clg2Setpt, targetRHsummer);

%add enthalpy
obj.BuildingWeatherTable.enthalpy = ...
    ece.Building.calculateEnthalpyAirH20(obj.BuildingWeatherTable.DryBulb_F, ...
    obj.BuildingWeatherTable.Rhum);

%Calculating the total heating °F-hrs. negatives not included.
obj.BuildingWeatherTable.htgFhrs1 = max(0, htg1Setpt-obj.BuildingWeatherTable.DryBulb_F);
obj.BuildingWeatherTable.htgFhrs2=max(0, htg2Setpt-obj.BuildingWeatherTable.DryBulb_F);
%Calculating the total cooling °F-hrs
%utilization factor method includes negatives, i.e. it is the net cooling degree hours
obj.BuildingWeatherTable.clgFhrs1 = obj.BuildingWeatherTable.DryBulb_F - clg1Setpt;
obj.BuildingWeatherTable.clgFhrs2 = obj.BuildingWeatherTable.DryBulb_F - clg2Setpt;
%Calculating the total enthalpy °F-hrs
%utilization factor method includes negatives
obj.BuildingWeatherTable.enHrs1 = obj.BuildingWeatherTable.enthalpy - en1Setpt;
obj.BuildingWeatherTable.enHrs2 = obj.BuildingWeatherTable.enthalpy - en2Setpt;
...
x = obj.BuildingWeatherTable.dayNumber >= htgStrtDayNumber | obj.BuildingWeatherTable.dayNumber < htgEndDayNumber;
obj.BuildingWeatherTable.htgFhrs1 = obj.BuildingWeatherTable.htgFhrs1 .* x;
obj.BuildingWeatherTable.htgFhrs2 = obj.BuildingWeatherTable.htgFhrs2 .* x;
y = obj.BuildingWeatherTable.dayNumber >= clgStrtDayNumber & obj.BuildingWeatherTable.dayNumber < clgEndDayNumber;
obj.BuildingWeatherTable.clgFhrs1 = obj.BuildingWeatherTable.clgFhrs1 .* y;
obj.BuildingWeatherTable.clgFhrs2 = obj.BuildingWeatherTable.clgFhrs2 .* y;
obj.BuildingWeatherTable.enHrs1 = obj.BuildingWeatherTable.enHrs1 .* y;
obj.BuildingWeatherTable.enHrs2 = obj.BuildingWeatherTable.enHrs2 .* y;

htgTime1 = obj.BuildingWeatherTable.Hour >= htg1StrtTime & obj.BuildingWeatherTable.Hour < htg1EndTime;
htgTime2 = obj.BuildingWeatherTable.Hour < htg1StrtTime | obj.BuildingWeatherTable.Hour >= htg1EndTime;
clgTime1 = obj.BuildingWeatherTable.Hour >= clg1StrtTime & obj.BuildingWeatherTable.Hour < clg1EndTime;
clgTime2 = obj.BuildingWeatherTable.Hour < clg1StrtTime | obj.BuildingWeatherTable.Hour >= clg1EndTime;
...
for n=1:12
    z = obj.BuildingWeatherTable.Month == n;
    hdd1(n) = sum(obj.BuildingWeatherTable.htgFhrs1 .* z .* htgTime1) / 24;
    hdd2(n) = sum(obj.BuildingWeatherTable.htgFhrs2 .* z .*htgTime2) / 24;
    cdd1(n) = sum(obj.BuildingWeatherTable.clgFhrs1 .* z .* clgTime1) / 24;
    cdd2(n) = sum(obj.BuildingWeatherTable.clgFhrs2 .* z .* clgTime2) / 24;
    edd1(n) = sum(obj.BuildingWeatherTable.enHrs1 .* z .* clgTime1) / 24;
    edd2(n) = sum(obj.BuildingWeatherTable.enHrs2 .* z .* clgTime2) / 24;
    %add average temperature, enthalpy, and wind speed. for all months for
    %both time periods
    avgTemp1(n) = sum(obj.BuildingWeatherTable.DryBulb_F .* z .* htgTime1) / ...
        sum(z .* htgTime1);
    avgTemp2(n) = sum(obj.BuildingWeatherTable.DryBulb_F .* z .* htgTime2) / ...
        sum(z .* htgTime2);
    avgEnthalp1(n) = sum(obj.BuildingWeatherTable.enthalpy .* z .* clgTime1) / ...
        sum(z .* clgTime1);
    avgEnthalp2(n) = sum(obj.BuildingWeatherTable.enthalpy .* z .* clgTime2) / ...
        sum(z .* clgTime2);
    avgWspeed1(n) = sum(obj.BuildingWeatherTable.wSpeed_mph .* z .* htgTime1) / ...
        sum(z .* htgTime1);
    avgWspeed2(n) = sum(obj.BuildingWeatherTable.wSpeed_mph .* z .* htgTime2) / ...
        sum(z .* htgTime2);
end
