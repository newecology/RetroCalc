function plotActualVModeledMonthlyWaterUsage(obj, ax)
% plotActualVModeledMonthlyWaterUsage: Method to plot the actual
% vs. Modeled monthly usage of Water for a single year.
%   This method pulls the Actual (HEA) and Modeled (L2) electric
...
% Acquire total actual Water usage data from HEA (gallons)
actualUsage = obj.MonthlyWaterProfile.TotalGallons;

% Acquire total modeled Water usage data from Level 2 (gallons)
%   This value is taken from the last row of the Water Usage Table
modeledUsage = obj.WaterUsageTable{end,2:13}';
...
% -- Axes Labels
title(ax,"Actual vs. Modelled Monthly Water Usage");
xlabel(ax,"Month");
