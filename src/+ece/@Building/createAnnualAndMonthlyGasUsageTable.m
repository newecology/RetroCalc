function createAnnualAndMonthlyGasUsageTable(bldg,gasMeters,gasRatios,...
    numYearsToAvg)
...
avgNumberOfBRs = sum(bldg.NumberOfBedroomUnits .* [1,2,3,4]) / ...
    bldg.NumberOfUnits;
...
for meterIdx = 1:numMeters
    ...
    % Note: The indices pulled are columns from AdjTherms and beyond, and also
    % including the initial Therms.
    propAdjustedUsageTable = gm.AdjustedUsageTable(:,...
        ["Usage","Cost","AdjTherms","DHWTherms"]);
    ...
    if (gm.IsCommonAreaClothesDryer)
    ...
    % Scale this vector based on proportional meter usage.
    gasStoveDryerTotalAnnlTherms = gasStoveDryerTotalAnnlTherms .* ...
        gmProp;
...
% Clean any NaN or Infs
monthlyProfile(isnan(monthlyProfile)) = 0;
monthlyProfile(isinf(monthlyProfile)) = 0;
