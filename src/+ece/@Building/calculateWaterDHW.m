...
numUnits = obj.NumberOfUnits;
numOcc = obj.NumberOfOccupants;
...
for dhwIdx = 1:numDHWsystems
    heaterType(dhwIdx) = obj.DHWsystems(dhwIdx).WaterHeaterType;
    recircYorN(dhwIdx) = obj.DHWsystems(dhwIdx).DHWrecirculation;
    recirType(dhwIdx) = obj.DHWsystems(dhwIdx).RecircLoopType;
    effDHW(dhwIdx) = obj.DHWsystems(dhwIdx).SteadyStateEfficiency;
    effDHWamp(dhwIdx) = obj.DHWsystems(dhwIdx).EfficiencySeasonalAmplitude;
    circLoss(dhwIdx) = obj.DHWsystems(dhwIdx).CircLossesJulyFracOfLoad;
    circLossAmp(dhwIdx) = obj.DHWsystems(dhwIdx).CircLossesSeasonalAmplitude;
    circFracCond(dhwIdx) = obj.DHWsystems(dhwIdx).CircLossesFracCond;
    controlskW(dhwIdx) = obj.DHWsystems(dhwIdx).ControlskW;
    fracLoadServed(dhwIdx) = obj.DHWsystems(dhwIdx).FractionBuildingLoadServed;
    
end % forloop (numDHWSystems)
...
loss = zeros(length(obj.DHWtanks),1);
tankQty = 0;
...
waterKsink = 0; 
waterShower = 0;
waterInUnitDishwasher = 0;
waterInUnitClotheswasher = 0; 
waterCommonClotheswasher = 0;
waterCommercialKitchenSink = 0; 
waterCommercialDishwasher = 0;
waterIrrigation = 0;
waterCoolingTower = 0;
waterOther = 0;
...
for n = 1:numDHWpipes
    ...
    elseif   obj.DHWpipesMechRoom(n).PipeType == 'SteelInsul'
        DHWpipeLoss(n) = obj.DHWpipesMechRoom(n).Length_ft * ...
            PipeHeatLoss140FTbl.SteelInsul(PipeHeatLoss140FTbl.PipeDiameter_Inch == ...
            obj.DHWpipesMechRoom(n).PipeDiameter_inch) * ...
            obj.DHWpipesMechRoom(n).HoursHot / 1000;
    else
        % MW_MISU: Need to handle a case where there is a pipe type that is
        % NOT accounted for.
...
for n = 1:numDHWsystems
    ...
    elseif obj.DHWsystems(n).WaterHeaterType == "PropaneFiredHeaterWithIndirectTank" || ...
        ...
        DHWInput12_kWh(n, :) = DHWheaterInputEnergy12(n, :) * 1000 ...
...
obj.DHWcontrolsTable = DHWcontrolsTbl_kWh;
