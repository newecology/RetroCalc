...
arguments
    ...
    intGainsHtgSeason12 (12,1) double
    solarGainsHtgSeason12  (12,1) double
end %argblock
...
intGainsUtilHtg = zeros(12,1);
for monthIdx = 1:12
    if IGLRh(monthIdx) <= .7
        intGainsUtilHtg(monthIdx) = 1;

    elseif IGLRh(monthIdx) > .7 && IGLRh(monthIdx) <= 5
        intGainsUtilHtg(monthIdx) = (.675 + 2.358 * IGLRh(monthIdx)^2.342) / (1 + 2.358 * IGLRh(monthIdx)^(2.342+1));
    
    else
        intGainsUtilHtg(monthIdx) = 1 / IGLRh(monthIdx);
    end
