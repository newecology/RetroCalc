...
arguments
    % obj: Self-referential Propnae utility object.
    obj (1,1) ece.Propane
...
%% Compute Minumum Adjusted Propnae Therm Usage Per Year
...
minPropaneGallonPerYear = nan(obj.NumMonthsOfData,1);
...
for yearIdx = 1:obj.NumberOfYears
    ...
    monthlyAdjGallonUsage = obj.AdjustedUsageTable.AdjGallons(monthIndices);

    % Obtain the average of the two lowest values in the array.
    avgLowestGallon = mean(mink(monthlyAdjGallonUsage,2));

    % Store into preallocated array via masking.
    minPropaneGallonPerYear(monthIndices) = avgLowestGallon;

end %forloop

% Clear any remaining NaN values (though, logically, there shouldn't be
% any)
minPropaneGallonPerYear(isnan(minPropaneGallonPerYear)) = [];

% Compute Average PropnaeThermOverall
minPropaneOverallAvg = mean(minPropaneGallonPerYear,"omitmissing");
...
obj.AdjustedUsageTable.DHWGallons= zeros(obj.NumMonthsOfData,1);

% Check if Propnae meter is used for DHW to calculate full DHWTherms.
if (obj.IsDHW)
    % Compute DHW Therm
    ...
    % Assign to Therms
    obj.AdjustedUsageTable.DHWGallons = dhwGallons;
