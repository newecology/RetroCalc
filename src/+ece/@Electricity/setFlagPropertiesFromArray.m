function setFlagPropertiesFromArray(elecUtil, propArray)
...
arguments
    % elecUtil: Self-referential Electricity object.
    elecUtil (1,1) ece.Electricity
...
elecUtil.NumberOfYears = propArray(1);

% 2) IsBaseLoad
% Convert to logical (0 = false, any other value is true)
elecUtil.IsBaseLoad = logical(propArray(2));

% 3) IsSpaceHeat
% Convert to logical (0 = false, any other value is true)
elecUtil.IsSpaceHeat = logical(propArray(3));

% 4) IsCooling
% Convert to logical (0 = false, any other value is true)
elecUtil.IsCooling = logical(propArray(4));

% 5) IsDHW
% Convert to logical (0 = false, any other value is true)
elecUtil.IsDHW = logical(propArray(5));

% 6) PayerType
% Map numeric value to PayerType enumeration member.
elecUtil.UtilityPayerType = ...
    ece.enum.UtilityPayerType.fromNumber(propArray(6));

% 7) ServiceType
% Map numeric value to ServiceType enumeration member.
elecUtil.UtilityServiceType = ...
    ece.enum.UtilityServiceType.fromNumber(propArray(7));

% 8) RealEstateType
% Map numeric value to RealEstateType enumeration member.
elecUtil.RealEstateType = ....
    ece.enum.RealEstateType.fromNumber(propArray(8));

% 9) BaseAdjustment
% Direcly assign double value.
elecUtil.ElecBaseAdj = propArray(9);

% 10) BaseElecAmplitude
% Direcly assign double value.
elecUtil.BaseElecAmplitude = propArray(10);

% 11) SeasonalAmpDHWUse
% Direcly assign double value.
elecUtil.SeasonalAmpDHWUse = propArray(11);
