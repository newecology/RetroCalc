function elecUtilities = fromUtilityExcelFile(excelFilePath)
...
if (numElecSheets == 0)
    ...
    elecUtilities = ece.Electricity.empty(0,1);
    return;
end %endif
...
elecUtilities = ece.Electricity.empty(numElecSheets,0);
...
for sheetIdx = 1:numElecSheets
    ...
    elecUtil.setFlagPropertiesFromArray(propArray);   
    ...
    elecUtil.HeatFractionLimitsTable = readtable(excelFilePath,opts);

    %% Store Constructed Electricity Object
    % Place into position in initialized array.
    elecUtilities(sheetIdx) = elecUtil;
