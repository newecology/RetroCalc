...
% CALCULATEBGSURFACEHEATLOSS Method to calculate heat loss from below grade
% surfaces, i.e. basement or crawl space.
%   When this method is called, the BG surface object will calculate and 
%   update the BGWall and BGfloor heat losses for the building.
...
wallR = (obj.BaseSlabR + obj.BGwallInsulR) .* 1.0 .* ...
...
groundSurfaceTemp = obj.Location.GroundMeanAnnualTemp_F - ...
    obj.Location.GroundSurfaceTempAmplitude_F * cos(360/365 ...
...
BGWallMonthSegmentHL = BGwallMonthSegmentHLC .* ...
    (wallVerticalSegment .* obj.BasementPerimeter_ft) .* ...
    (daysMonth * 24) / 1000;

% basement floor heat loss in kBtu for each segment, each month
BGfloorMonthSegmentHL = BGfloorMonthSegmentHLC .* ...
    obj.BGfloorArea_ft2 .* ...
    (daysMonth * 24) / 1000;
