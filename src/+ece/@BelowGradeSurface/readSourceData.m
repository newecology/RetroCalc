...
BGMask = contains(sheetNames,"BGsurfaces");
...
opts = detectImportOptions(fileName,...
    ...
    "Sheet",BGSheetNames(1),...
    "Range","A2:M7");
BGTbl = readtable(fileName,opts);
% Remove rows where the below columns are missing values.
BGTbl = rmmissing(BGTbl,"DataVariables",...
    ["BGwallInsulR","BGwallArea_ft2"]);
...
%% Get Soil Conductivity Table from the Reference Table
% Used to compute values for BelowGradeSurface object.
SoilConductivityTbl=ece.Reference.SoilConductivityTable;

%% Iterate through each row to assign properties
% Populate the BelowGradeSurface array with the values in the table.
...
for bgIdx = 1:numBG
    ...
    bg.SoilThermalConductivity = SoilConductivityTbl.Value(...
        SoilConductivityTbl.SoilType == BGTbl.SoilThermalConductivity(bgIdx));
    
    bg.BaseSlabR = SoilConductivityTbl.Value(...
        strcmpi(SoilConductivityTbl.SoilType,...
