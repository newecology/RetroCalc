...
numMonths = numel(gasUtil.AdjustedUsageTable.StartDate);

% Month and day for the first and last month start and end dates.
firstStartDateM = month(gasUtil.AdjustedUsageTable.StartDate(1));
firstStartDateD = day(gasUtil.AdjustedUsageTable.StartDate(1));
firstEndDateM = month(gasUtil.AdjustedUsageTable.EndDate(1));
firstEndDateD = day(gasUtil.AdjustedUsageTable.EndDate(1));

lastStartDateM = month(gasUtil.AdjustedUsageTable.StartDate(end));
lastStartDateD = day(gasUtil.AdjustedUsageTable.StartDate(end));
lastEndDateM = month(gasUtil.AdjustedUsageTable.EndDate(end));
lastEndDateD = day(gasUtil.AdjustedUsageTable.EndDate(end));
...
gasUtil.AdjustedUsageTable.Month = monthSequence';

% Years Value
yearVals = year(gasUtil.AdjustedUsageTable.StartDate + days(16));
gasUtil.AdjustedUsageTable.Year = yearVals;

%% Adjust therm Values
% The therms values need to be proportioned based on the new times. Put this
...
% are scaled up or down to ensure they sum to the same total therms as before.

% Convert to a monthly form of adjusted therms.
gasUtil.AdjustedUsageTable.AdjTherms = (gasUtil.AdjustedUsageTable.Usage ./ ...
    gasUtil.AdjustedUsageTable.Days) * (365/12);
...
% Apply adjustment to Adjtherms to make the total sum usage the same.
