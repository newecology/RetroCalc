...
arguments
    % obj: Self-referential Oil utility object.
    obj (1,1) ece.Oil
...
%% Compute Minumum Adjusted Oil Therm Usage Per Year
...
minOilGallonPerYear = nan(obj.NumMonthsOfData,1);
...
for yearIdx = 1:obj.NumberOfYears
    ...
    % Extract adjusted Gallon for provided months.
    ...
    minOilGallonPerYear(monthIndices) = avgLowestGallon;

end %forloop

% Clear any remaining NaN values (though, logically, there shouldn't be
% any)
minOilGallonPerYear(isnan(minOilGallonPerYear)) = [];

% Compute Average OilThermOverall
minOilOverallAvg = mean(minOilGallonPerYear,"omitmissing");
...
% Check if Oil meter is used for DHW to calculate full DHWTherms.
if (obj.IsDHW)
    % Compute DHW Therm
    dhwGallons = minOilOverallAvg * ...
