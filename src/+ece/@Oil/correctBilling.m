function correctBilling(oilUtil)
...
% representations.
% Days Value
deltaDays = days(oilUtil.AdjustedUsageTable.EndDate - oilUtil.AdjustedUsageTable.StartDate);
oilUtil.AdjustedUsageTable.Days = deltaDays + 1;
...
numMonths = numel(oilUtil.AdjustedUsageTable.StartDate);

% Month and day for the first and last month start and end dates.
firstStartDateM = month(oilUtil.AdjustedUsageTable.StartDate(1));
firstStartDateD = day(oilUtil.AdjustedUsageTable.StartDate(1));
firstEndDateM = month(oilUtil.AdjustedUsageTable.EndDate(1));
firstEndDateD = day(oilUtil.AdjustedUsageTable.EndDate(1));

lastStartDateM = month(oilUtil.AdjustedUsageTable.StartDate(end));
lastStartDateD = day(oilUtil.AdjustedUsageTable.StartDate(end));
lastEndDateM = month(oilUtil.AdjustedUsageTable.EndDate(end));
lastEndDateD = day(oilUtil.AdjustedUsageTable.EndDate(end));
...
oilUtil.AdjustedUsageTable.Month = monthSequence';

% Years Value
yearVals = year(oilUtil.AdjustedUsageTable.StartDate + days(16));
oilUtil.AdjustedUsageTable.Year = yearVals;

%% Adjust gallonsValues
...
oilUtil.AdjustedUsageTable.AdjGallons = (oilUtil.AdjustedUsageTable.Usage ./ oilUtil.AdjustedUsageTable.Days) * (365/12);

% Compute the fractional adjustment needed to map sum of AdjTherms back
% to total real therms.
fracAdj = sum(oilUtil.AdjustedUsageTable.AdjGallons) / sum(oilUtil.AdjustedUsageTable.Usage);

% Apply adjustment to AdjkWh to make the values the same.
oilUtil.AdjustedUsageTable.AdjGallons = oilUtil.AdjustedUsageTable.AdjGallons / fracAdj;
