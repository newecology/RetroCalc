...
% Months Value
monthVals = month(oilUtil.AdjustedUsageTable.StartDate + days(15));
oilUtil.AdjustedUsageTable.Month = monthVals;

% Years Value
yearVals = year(oilUtil.AdjustedUsageTable.StartDate + days(15));
...
for mIdx = 2:oilUtil.NumMonthsOfData
    % -- Get Logical Flags
    % Current month matches last previous month in table.
    isCurrentMonthSameAsLastMonth = ...
        oilUtil.AdjustedUsageTable.Month(mIdx) == oilUtil.AdjustedUsageTable.Month(mIdx-1);
    % Current month matches next month, but in a weird way?
    isCurrentMonthSameAsNextMonth = ...
        oilUtil.AdjustedUsageTable.Month(mIdx) == oilUtil.AdjustedUsageTable.Month(mIdx-1) + 2;
