...
numMonths = numel(waterUtil.AdjustedUsageTable.StartDate);

% Month and day for the first and last month start and end dates.
firstStartDateM = month(waterUtil.AdjustedUsageTable.StartDate(1));
firstStartDateD = day(waterUtil.AdjustedUsageTable.StartDate(1));
firstEndDateM = month(waterUtil.AdjustedUsageTable.EndDate(1));
firstEndDateD = day(waterUtil.AdjustedUsageTable.EndDate(1));

lastStartDateM = month(waterUtil.AdjustedUsageTable.StartDate(end));
lastStartDateD = day(waterUtil.AdjustedUsageTable.StartDate(end));
lastEndDateM = month(waterUtil.AdjustedUsageTable.EndDate(end));
lastEndDateD = day(waterUtil.AdjustedUsageTable.EndDate(end));
...
waterUtil.AdjustedUsageTable.Month = monthSequence';

% Years Value
yearVals = year(waterUtil.AdjustedUsageTable.StartDate + days(16));
