function waterUtilities = fromUtilityExcelFile(excelFilePath)
...
if (numWaterSheets == 0)
    ...
    waterUtilities = ece.Water.empty(0,1);
    return;
end %endif
...
waterUtilities = ece.Water.empty(numWaterSheets,0);
...
for sheetIdx = 1:numWaterSheets
    ...
    waterUtil.setFlagPropertiesFromArray(propArray);   
    ...
    waterUtil.importUsageTable(rawUsageTable);

    %% Store Constructed Water Object
    % Place into position in initialized array.
    waterUtilities(sheetIdx) = waterUtil;
